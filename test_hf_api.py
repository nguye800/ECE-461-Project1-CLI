...
def parse_hf_url(url: str):
    ...
    path_parts = _strip_empty(parsed.path.split("/"))
    if not path_parts:
...
def build_api_url(kind: str, repo_id: str) -> str:
    if kind == "model":
...
def fetch_json(api_url: str, token: str | None):
...
def main():
    ...
    try:
        kind, repo_id = parse_hf_url(args.url)
    except ValueError as e:
        print(f"Error: {e}", file=sys.stderr)
        sys.exit(2)

    api_url = build_api_url(kind, repo_id)
